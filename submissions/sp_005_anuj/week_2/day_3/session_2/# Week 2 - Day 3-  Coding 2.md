...
1. Inherited
```
`color` `text-align` `font-size` `font-weight` `visibility`
```
2. Non-Inherited
```
`padding` `background-color` `border` `display` `float` `text-align` `width` `overflow` `word-wrap` `z-index` `position` `clear`
```
